...
def grpo_train_vanilla(
    ...
    **kwargs: Any,
    ...
    trainer_config = build_trainer_args(
        training_cfg, dataset_size=len(dataset), **kwargs
    )
    ...
    def _make_named_reward_func(base_reward_func, **reward_kwargs):
        def _wrapped(prompts, completions, **trainer_kwargs):
            # Forward trainer kwargs (e.g., training step) to the base reward for logging/gating
            return base_reward_func(
                prompts, completions, **reward_kwargs, **trainer_kwargs
            )
