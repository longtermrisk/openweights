...
def install_colocated_vllm_generate_from_trainer(trainer) -> None:
    ...
    # Allow auto mode or any mode when the colocated llm is present
    if not use_vllm or not hasattr(trainer, "llm"):
        return
    if vllm_mode not in ("colocate", "auto", None):
        return
    ...
    def generate_fn(prompts, **gen_kwargs):
        ...
        print("Example norm prompts[0]:\n", norm_prompts[0], flush=True)
...
def get_completions(prompts: List[str], generation_kwargs: Dict[str, Any]) -> List[str]:
    ...
    if use_colocated and _colocated_vllm_generate_fn is not None:
        raise NotImplementedError(
            "(Not sure) System prompt not supported for colocated vLLM generate hook"
        )
    ...
    def _single_completion(prompt: str) -> str:
        ...
        #             try:
        #                 text_prompt = "\n".join(
        #                     [f"{m['role'].upper()}: {m['content']}" for m in prompt]
        #                 )
        #             except Exception:
        ...
        if isinstance(prompt, str):
            ...
            raise ValueError(f"Prompt must be a string or list of dicts: {prompt}")

        # Normalize system_prompt in case someone passed a message list there too
        sys_prompt_text = system_prompt
        assert isinstance(sys_prompt_text, str), "System prompt must be a string"
        # if (
...
def check_no_CoT_judge(judge_prompt_name: Optional[str], judge_model: Optional[str]):
    name = (judge_prompt_name or "").lower().replace("_", "").replace("-", "")
    model = (judge_model or "").lower().replace("_", "").replace("-", "")
    return "nocot" in name and "gpt5" not in model
